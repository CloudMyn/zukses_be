...
class ChatMessageUpdateRequest extends FormRequest
{
    ...
    {
        $id = $this->route('chatMessage'); // Get the chatMessage ID from the route

        return [
            'id_obrolan' => 'sometimes|required|exists:obrolan,id',
            'id_pengirim' => 'sometimes|required|exists:users,id',
            'isi_pesan' => 'required_if:jenis_pesan,TEKS|string|max:1000',
            'jenis_pesan' => 'sometimes|required|in:TEKS,GAMBAR,VIDEO,DOKUMEN,LOKASI',
