...
class AdminUserUpdateRequest extends FormRequest
{
    ...
    {
        $id = $this->route('adminUser'); // Get the adminUser ID from the route

        return [
            'id_user' => 'sometimes|required|exists:users,id',
            'nip' => 'sometimes|required|string|unique:admin_users,nip,' . $id,
            'nama_lengkap' => 'sometimes|required|string|max:255',
            'nomor_telepon' => 'sometimes|required|string|max:20',
            'departemen' => 'sometimes|required|string|max:100',
            'jabatan' => 'sometimes|required|string|max:100',
            'tanggal_mulai_bekerja' => 'sometimes|required|date',
