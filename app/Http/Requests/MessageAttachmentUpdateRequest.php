...
class MessageAttachmentUpdateRequest extends FormRequest
{
    /**
    ...
    {
        $id = $this->route('messageAttachment'); // Get the messageAttachment ID from the route

        return [
            'id_pesan' => 'sometimes|required|exists:pesan_obrolan,id',
            'nama_file' => 'sometimes|required|string|max:255',
            'path_file' => 'sometimes|required|string|max:500',
            'url_file' => 'sometimes|required|url',
            'jenis_file' => 'sometimes|required|in:GAMBAR,VIDEO,AUDIO,DOKUMEN,LAINNYA',
            'ukuran_file' => 'sometimes|required|integer|min:0',
