...
class ProductReviewUpdateRequest extends FormRequest
{
    ...
    {
        $id = $this->route('productReview'); // Get the productReview ID from the route

        return [
            'id_produk' => 'sometimes|required|exists:tb_produk,id',
            'id_order_item' => 'sometimes|required|exists:tb_order_item,id',
            'id_varian_produk' => 'nullable|exists:varian_produk,id',
            'rating' => 'sometimes|required|integer|min:1|max:5',
            'judul_ulasan' => 'sometimes|required|string|max:255',
            'isi_ulasan' => 'sometimes|required|string',
            'is_verified_purchase' => 'nullable|boolean',
            'is_rekomendasi' => 'nullable|boolean',
            'status_ulasan' => 'sometimes|required|in:DRAFT,AKTIF,TIDAK_AKTIF,HAPUS,DITOLAK',
        ];
    }
    ...
    public function messages(): array
    {
        return [
            ...
            'status_ulasan.in' => 'Status ulasan tidak valid',
        ];
    }
}