        ...
        return [
            'id_metode_pengiriman' => [
                ...
                Rule::unique('tarif_pengiriman', 'id_metode_pengiriman')
                    ->where('id_asal', $this->id_asal ?? $this->route('shipping_rate')->id_asal)
                    ->where('id_tujuan', $this->id_tujuan ?? $this->route('shipping_rate')->id_tujuan)
                    ->ignore($this->route('shipping_rate')?->id)
            ],
            'id_asal' => 'sometimes|required|integer',
            'id_tujuan' => 'sometimes|required|integer',
            'tipe_jarak' => 'sometimes|required|in:LOKAL,ANTAR_KOTA,ANTAR_PROVINSI,INTERNASIONAL',
            'min_berat' => 'sometimes|required|numeric|min:0',
            'max_berat' => 'sometimes|required|numeric|min:0|gte:min_berat',
            'min_nilai' => 'sometimes|required|numeric|min:0',
            'max_nilai' => 'sometimes|required|numeric|min:0|gte:min_nilai',
            'biaya_dasar' => 'sometimes|required|numeric|min:0',
            'biaya_per_kg' => 'sometimes|required|numeric|min:0',
            'estimasi_hari' => 'sometimes|required|integer|min:1',
            'is_aktif' => 'boolean',
            'prioritas' => 'sometimes|required|integer|min:0',
